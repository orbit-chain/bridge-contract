...
contract ERCDeployer is Context {
    ...
    function getVersion() public pure returns(string memory) {
        return "ERCTokenDeployer20210628";
    }
    ...
    function deployToken(uint8 decimals) public onlyMinter returns (address){
        ...
        return address(new ERC20(owner(), minter(), name, "OBT", decimals, false));
    }
    ...
    function deployNFT() public onlyMinter returns (address){
        ...
        return address(new ERC721Full(owner(), minter(), name, "OBN", false));
    }

    function deployNFTWithInit(string memory name, string memory symbol) public onlyMinter returns (address){
        return address(new ERC721Full(owner(), minter(), name, symbol, true));
    }
}
